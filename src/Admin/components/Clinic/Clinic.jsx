...
const AdminClinicTable = () => {
  const { adminClinic, loading, error, getAdminClinic, approveClinic, rejectClinic } = useContext(MyContext);
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      const result = await rejectClinic(selectedClinic._id, rejectReason);
  ...
  // Safe array check
  const clinicsArray = Array.isArray(adminClinic)
    ? adminClinic
    : adminClinic?.clinics || adminClinic?.data?.clinics || [];
  ...
  if (error) {
    return (
      <div className="alert alert-danger mt-4" role="alert">
        <h4 className="alert-heading">Error Loading Clinics</h4>
        <p className="mb-3"><strong>Error:</strong> {error}</p>
        <button className="btn btn-sm btn-danger" onClick={getAdminClinic}>
          Retry Loading Data
        </button>
      </div>
    );
  ...
  if (!clinicsArray || clinicsArray.length === 0) {
    return (
      <div className="alert alert-warning mt-4" role="alert">
        ...
        <button className="btn btn-sm btn-warning" onClick={getAdminClinic}>
          Retry Loading Data
        </button>
      </div>
    );
                ...
                <tr key={clinic._id || index}>
                  ...
                  <td>
                    <div className="d-flex gap-1 flex-wrap">
                      ...
                      <Link to={`/dashboard/admin/clinic-more-info/${clinic._id}`}>
