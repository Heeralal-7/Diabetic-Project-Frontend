      ...
      <div id={`${id}`} className="carousel slide carousel-fade" data-bs-ride={autoplay}>
        ...
        <div className="carousel-inner FoodAndNutrition CarouselHeight rounded-4">
          {slideData.map((slide, index) => (
            <div key={index} className={`carousel-item h-100 ${index === 0 ? 'active' : ''}`} data-bs-interval="4000">
              <div className="FoodAndNutriCarouselBg" style={{ backgroundImage: `linear-gradient(267deg,
                    rgba(255,255,255,0.1307057584269663) 33%, rgba(0,0,1,1) 100%),url(${slide.image})` }}></div>
              <div className="carousel-caption FoodAndNutriCarouselCaption">
                ...
                <button className=" btn text-light btn-lg  bg-mainRed animated-2 mb-1 fadeInUp">{slide.buttonText}</button>
              </div>
            </div>
          ))}
        </div>
        {/* Controls */}
        <button className="foodNNutrCarouselBtn carousel-control-prev rounded-4" href={`#${id}`} data-bs-slide="prev">
          <i className="ri-arrow-left-double-line display-5"></i>
        </button>
        <button className="foodNNutrCarouselBtn carousel-control-next rounded-4" href={`#${id}`} data-bs-slide="next">
