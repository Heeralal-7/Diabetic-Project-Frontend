...
const LabRevenue = ({ refreshKey }) => {
  ...
  const getAllVendorToken = useCallback(() => {
    const tokenNames = ['labtoken', 'Pharmacytoken', 'foodtoken'];
  ...
  const getStatusBadge = (status) => {
    ...
    if (s === '8' || s === '7' || s === 'completed') return <Badge bg="success">Completed</Badge>;
    if (s === '5' || s === 'delivered') return <Badge bg="success">Delivered</Badge>;
    if (s === '1' || s === 'confirmed') return <Badge bg="info">Confirmed</Badge>;
    if (s === '2' || s === 'processing') return <Badge bg="primary">Processing</Badge>;
  ...
  const fetchOrderRevenueData = async () => {
    try {
      ...
      const statsPromise = axios.get(`${URL}/lab-vendor-revenue/stats`, { headers });
      const ordersPromise = axios.get(`${URL}/lab-vendor-revenue/orders?limit=2000`, { headers });
      ...
      if (statsRes.data.success) {
        ...
        // Safe assignment: ensure values are not undefined
        setRevenueData({
  ...
  const fetchEligiblePayouts = async () => {
      try {
          ...
          const res = await axios.get(`${URL}/vendor-payout/payout-eligible`, {
              params: { type: type }, // 'lab'
              headers: { Authorization: `Bearer ${token}` }
          });
          
          if(res.data.success) {
              setEligibleOrders(res.data.orders || []);
              // Use totalPayableAmount if available, else totalAmount, else 0
              setPayoutStats({
  ...
  const getSelectedTotal = () => {
      return eligibleOrders
        ...
        .reduce((sum, current) => sum + (parseFloat(current.payableAmount || current.amount) || 0), 0);
  };
        ...
        <div className="col-12">
          ...
          {payoutTab === 'analytics' ? (
            <>
              ...
              {/* Recent Orders */}
              <div className="row"><div className="col-12"><div className="card shadow-sm"><div className="card-header bg-white"><h5 className="mb-0">Recent Order History</h5></div><div className="card-body p-0"><div className="table-responsive"><Table hover className="mb-0 align-middle"><thead className="bg-light"><tr><th>Order ID</th><th>Date</th><th>Customer</th><th>Status</th><th className="text-end">Total</th><th className="text-end text-danger">Admin</th><th className="text-end text-success">Net</th></tr></thead><tbody>{revenueData.recentOrders.length > 0 ? (revenueData.recentOrders.map((o,i)=>(<tr key={i}><td className="fw-bold text-primary">{o.orderId || o._id.slice(-8)}</td><td><div>{o.displayDate}</div><small className="text-muted">{o.displayTime}</small></td><td>{o.customerName}</td><td>{getStatusBadge(o.status)}</td><td className="text-end fw-bold">₹{(o.gross || 0).toFixed(2)}</td><td className="text-end text-danger">-₹{(o.admin || 0).toFixed(2)}</td><td className="text-end text-success fw-bold">₹{(o.net || 0).toFixed(2)}</td></tr>))) : (<tr><td colSpan="7" className="text-center py-4">No orders found.</td></tr>)}</tbody></Table></div></div></div></div></div>
            </>
          ) : (
            // === VIEW 2: PAYOUTS (NEW) ===
            <>
                ...
                {/* Summary Cards */}
                <div className="row mb-4">
                    <div className="col-md-4 mb-3"><div className="card bg-white border-start border-4 border-success shadow-sm h-100"><div className="card-body"><h6 className="text-muted text-uppercase mb-2">Available for Withdrawal</h6><h2 className="text-success fw-bold">₹{(payoutStats.totalAmount || 0).toFixed(2)}</h2><span className="badge bg-light text-dark border">{payoutStats.count} orders eligible</span></div></div></div>
