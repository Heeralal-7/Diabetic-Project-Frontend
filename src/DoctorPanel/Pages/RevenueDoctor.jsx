...
import { Alert, Spinner, Badge, Table, Button, Form, Modal, Card, Row, Col } from 'react-bootstrap';
...
const DoctorRevenue = ({ refreshKey }) => {
  // --- ANALYTICS STATE ---
  ...
  // --- INDEPENDENT DOCTOR CHECK STATE ---
  ...
  const getAllVendorToken = useCallback(() => {
    const tokenNames = ['doctortoken', 'labtoken', 'Pharmacytoken', 'foodtoken'];
  ...
  useEffect(() => {
    ...
    checkDoctorStatus(); // Check if doctor is independent or clinic managed
    // eslint-disable-next-line
  }, [refreshKey]);

  useEffect(() => {
    if (payoutTab === 'payout' && isIndependentDoctor) {
        fetchEligiblePayouts();
        fetchPayoutSummary();
        fetchPayoutHistory();
    }
    // eslint-disable-next-line
  }, [payoutTab, isIndependentDoctor]);

  // --- CHECK DOCTOR STATUS (CLINIC VS INDEPENDENT) ---
  const checkDoctorStatus = async () => {
    try {
        const { token, type } = getAllVendorToken();
        if (type !== 'doctor') return; // Only applies to doctors

        // We use the bank-details endpoint to check status as per the reference code logic
        ...
        // If error message says managed by clinic
  ...
  const getStatusBadge = (status) => {
    ...
    if (s === '7' || s === 'completed' || s === 'visited' || s === 'checked') return <Badge bg="success">Visited</Badge>;
  ...
  const fetchOrderRevenueData = async () => {
    try {
      ...
      const [statsRes, ordersRes] = await Promise.all([
        axios.get(`${URL}/doctor-revenue/stats`, { headers }),
        axios.get(`${URL}/doctor-revenue/orders?limit=2000`, { headers })
      ]);
      ...
      if (statsRes.data.success) {
        ...
        setRevenueData({
          totalRevenue: stats.total.revenue, monthlyRevenue: stats.monthly.revenue,
          weeklyRevenue: stats.weekly.revenue, dailyRevenue: stats.today.revenue, ordersCount: stats.total.count,
          totalNetEarning: calcTotalNet, totalAdminShare: calcTotalAdmin, currentCutoff: cutoff,
          revenueByDate, netEarningByDate, monthlyRevenueData: monthlyRevenueAggregated,
          monthlyNetData: monthlyNetAggregated, monthlyAdminData: monthlyAdminAggregated,
  ...
  const fetchEligiblePayouts = async () => {
      try {
          ...
          const res = await axios.get(`${URL}/vendor-payout/doctor/payout-eligible`, {
              params: { type: type }, 
              headers: { Authorization: `Bearer ${token}` }
          });
  ...
  const fetchPayoutSummary = async () => {
    try {
        const { token } = getAllVendorToken();
        const res = await axios.get(`${URL}/vendor-payout/doctor/payout-summary`, {
            headers: { Authorization: `Bearer ${token}` }
        });
        if(res.data.success) {
  ...
  const fetchPayoutHistory = async () => {
    try {
        const { token } = getAllVendorToken();
        const res = await axios.get(`${URL}/vendor-payout/doctor/payout-history`, {
            headers: { Authorization: `Bearer ${token}` }
        });
        if(res.data.success) {
  ...
  const submitPayoutRequest = async () => {
      ...
      try {
          ...
          await axios.post(`${URL}/vendor-payout/doctor/request-payout`, {
              vendorType: type,
  ...
  const barData = {
    ...
    datasets: [
      { label: 'Net Earning', data: sortedMonthLabels.map(m => revenueData.monthlyNetData[m]), backgroundColor: '#198754' },
      { label: 'Admin Share', data: sortedMonthLabels.map(m => revenueData.monthlyAdminData[m]), backgroundColor: '#dc3545' }
    ]
  };
  const lineData = {
    labels: lineChartSortedDates,
    datasets: [
      { label: 'Gross', data: lineChartSortedDates.map(d => revenueData.revenueByDate[d]), borderColor: '#0d6efd', fill: true, tension: 0.3 },
      { label: 'Net', data: lineChartSortedDates.map(d => revenueData.netEarningByDate[d]), borderColor: '#198754', fill: true, tension: 0.3 }
    ]
  };
  const pieData = {
    labels: ['Net', 'Admin'],
    datasets: [{ data: [revenueData.totalNetEarning, revenueData.totalAdminShare], backgroundColor: ['#198754', '#dc3545'] }]
  };
        ...
        <div className="col-12">
          ...
          {payoutTab === 'analytics' ? (
            <>
                <div className="row mb-4">
                    ...
                    <div className="col-md-3" key={type}>
                        ...
                        <div className="card-body">
                            <h5>{type} Revenue</h5>
                            <h2 className="fw-bold">₹{revenueData[`${type.toLowerCase()}Revenue`].toFixed(2)}</h2>
                ...
                <div className="row mb-4">
                    <div className="col-md-3"><div className="card border-primary h-100 shadow-sm"><div className="card-body"><h6 className="text-primary">Gross Revenue</h6><h3 className="fw-bold">₹{revenueData.totalRevenue.toFixed(2)}</h3></div></div></div>
                    <div className="col-md-3"><div className="card border-success h-100 shadow-sm"><div className="card-body"><h6 className="text-success">Net Earnings</h6><h3 className="fw-bold">₹{revenueData.totalNetEarning.toFixed(2)}</h3></div></div></div>
                    <div className="col-md-3"><div className="card border-danger h-100 shadow-sm"><div className="card-body"><h6 className="text-danger">Admin Commission</h6><h3 className="fw-bold">₹{revenueData.totalAdminShare.toFixed(2)}</h3></div></div></div>
                ...
                <div className="row"><div className="col-12"><div className="card shadow-sm"><div className="card-header bg-white"><h5 className="mb-0">Recent Appointments</h5></div><div className="card-body p-0"><div className="table-responsive"><Table hover className="mb-0 align-middle"><thead className="bg-light"><tr><th>ID</th><th>Date</th><th>Patient</th><th>Status</th><th className="text-end">Total</th><th className="text-end text-danger">Admin</th><th className="text-end text-success">Net</th></tr></thead><tbody>{revenueData.recentOrders.length > 0 ? revenueData.recentOrders.map((o,i)=>(<tr key={i}><td className="fw-bold text-primary">{o.orderId}</td><td><div>{o.displayDate}</div><small className="text-muted">{o.displayTime}</small></td><td>{o.customerName}</td><td>{getStatusBadge(o.status)}</td><td className="text-end fw-bold">₹{o.gross.toFixed(2)}</td><td className="text-end text-danger">-₹{o.admin.toFixed(2)}</td><td className="text-end text-success fw-bold">₹{o.net.toFixed(2)}</td></tr>)) : <tr><td colSpan="7" className="text-center py-4">No appointments found.</td></tr>}</tbody></Table></div></div></div></div></div>
            </>
          ) : (
            // === VIEW 2: PAYOUTS ===
            <>
                {/* 
                    LOGIC CHECK: 
                    If Independent Doctor -> Show Payout System
                    If Clinic Managed Doctor -> Show Warning (Same as Bank Settings)
                */}
                {!isIndependentDoctor ? (
                         ...
                         <div className="col-md-8">
                            <Card className="shadow-lg border-0 rounded-4">
                              <div className="card-header bg-warning text-dark py-3 rounded-top-4">
                                <h4 className="mb-0"><i className="bi bi-wallet2 me-2"></i> Payout Management</h4>
                              </div>
                              <Card.Body className="p-5 text-center">
                                <Alert variant="warning" className="mb-4">
                                  <i className="bi bi-exclamation-triangle-fill me-2 fs-5"></i>
                                  <h4 className="alert-heading d-inline">Managed by Clinic</h4>
                                  <p className="mt-3 mb-0 fs-5">
                                    Your payout requests and bank details are managed by your Clinic. <br/>
                                    Please contact your clinic administrator for any payout related queries.
                                  </p>
                                </Alert>
                                <div className="mt-4">
                                  <i className="bi bi-building text-muted mb-3" style={{ fontSize: '4rem'}}></i>
                                  <p className="text-muted">
                                    As a clinic-affiliated doctor, your earnings are processed through
                    ...
                    // --- INDEPENDENT DOCTOR PAYOUT VIEW ---
                    <>
