...
const DoctorSubadmin = () => {
  ...
  useEffect(() => {
    if (doctors && doctors.length > 0) {
      ...
      if (selectedState) {
        ...
        const allCities = [...new Set(doctors.map(doc => doc.city).filter(Boolean))];
  ...
  useEffect(() => {
    getDoctors({
      page: 1,
      limit: 100, // Get more doctors for filtering
      search: "",
      verification: "",
      specialist: ""
    });
  ...
  }, [doctors, searchTerm, selectedState, selectedCity, radius, userLocation]);
  ...
  const handleVerifyDoctor = async (doctorId) => {
    try {
      ...
      const result = await verifyDoctorAccount(doctorId);
      ...
      if (result && result.success) {
        ...
        toast.error(result.message || "Error approving doctor");
      }
    } catch (error) {
  ...
  const confirmRejectDoctor = async () => {
    ...
    try {
      ...
      const result = await rejectDoctorAccount(selectedDoctorId, rejectReason.trim());
      ...
      if (result && result.success) {
        ...
        toast.error(result.message || "Error rejecting doctor");
      }
    } catch (error) {
  ...
  const getActionButtons = (doc) => {
    ...
    return (
      <div className="d-flex align-items-center gap-2">
        ...
        <Link to={`/subadmin-dashboard/ViewDoctor/${doc._id}`}>
